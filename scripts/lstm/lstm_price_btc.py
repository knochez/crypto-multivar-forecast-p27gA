...
Original file is located at
    https://colab.research.google.com/drive/1eBAvVoDQBXD2WlfbEGKCahLQ8nsSIAnD
"""

"""
lstm1_price.py — BTC Close Price forecasting using LSTM
=====================================================
...
LSTM settings for price:
  - 128 hidden units, OneCycleLR, BatchNorm — aggressive settings
    work well for non-stationary price series
  - 2-layer stacked LSTM with two-layer FC head
  - drop_last=True prevents BatchNorm crash on incomplete final batch
"""
...
RESULTS_DIR  = "results_lstm_price"
...
class LSTMForecaster(nn.Module):
    """
    LSTM -> BatchNorm -> Dropout -> Linear(128->32) -> ReLU -> Linear(32->1)
    Stacked LSTM with two-layer FC head and BatchNorm for price forecasting.
    """
...
def plot_predictions(dates, yt, yp, lb, m):
    ...
    ax.plot(dates, yp, color="#D85A30", linewidth=1.1,
            ...
            label="Predicted btc close price (usd) — LSTM model one-day-ahead forecast")
    ax.set_title(
...
def main():
    ...
    print(f"  Hidden: {HIDDEN_SIZE}  |  Layers: {NUM_LAYERS}  |  Scheduler: OneCycleLR max_lr={MAX_LR}")
    print("=" * 55)
    ...
    for i, lb in enumerate(LOOKBACKS, 1):
        ...
        torch.save(model.state_dict(), os.path.join(
            DIRS["model_weights"],
            f"Model_Weights_LSTM_Price_lookback_{lb}d.pt"))
    ...
    csv = os.path.join(DIRS["summary"], "Results_LSTM_Price.csv")
