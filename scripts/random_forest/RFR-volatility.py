...
# Output folders
output_dir = Path("rf_volatility_outputs")
...
def volatility_mda(y_true, y_pred):
    y_true = np.asarray(y_true)
    y_pred = np.asarray(y_pred)
...
# -----------------------------
# 3. Regime windows
# Replace with your teammate's exact dates if needed
# -----------------------------
...
for coin in coins:
    print(f"\n{'='*80}")
    print(f"RUNNING RANDOM FOREST VOLATILITY PREDICTION FOR {coin}")
    print(f"{'='*80}")
    ...
    for lookback in lookbacks:
        ...
        mda = volatility_mda(y_test.values, y_pred)

        # -----------------------------
        # Save predictions CSV
        # -----------------------------
        results_df = test_df[["date"]].copy()
        results_df["actual_volatility"] = y_test.values
        results_df["predicted_volatility"] = y_pred
        results_df["error"] = results_df["actual_volatility"] - results_df["predicted_volatility"]

        pred_file = preds_dir / f"{coin.lower()}_rf_volatility_lb{lookback}.csv"
        ...
        subplot_data.append({
            ...
            "y_true": results_df["actual_volatility"].values,
            "y_pred": results_df["predicted_volatility"].values,
            "rmse": rmse,
            "mae": mae,
            "mape": mape,
            "mda": mda
    ...
    for ax, item in zip(axes, subplot_data):
        ...
        ax.plot(
            dates, y_true,
            color="#185FA5", linewidth=1.4
        )
        ax.plot(
            dates, y_pred,
            color="#D85A30", linewidth=1.1, linestyle="--", alpha=0.85
        )
    ...
    fig.suptitle(
        f"Random Forest Forecast — {coin} Volatility",
        fontsize=14,
        y=0.98
    )
    ...
    combined_file = graphs_dir / f"{coin.lower()}_rf_volatility_all_lookbacks.png"
...
print(f"\n{'='*80}")
print("FINAL VOLATILITY SUMMARY")
print(f"{'='*80}")
print(summary_df.sort_values(["coin", "lookback"]))

summary_file = output_dir / "rf_volatility_summary.csv"
