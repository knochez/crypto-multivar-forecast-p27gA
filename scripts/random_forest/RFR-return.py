...
# Tuned RF parameters from your return tuning
rf_params = {
    "n_estimators": 100,
    "max_depth": 5,
    "min_samples_split": 2,
...
# Output folders
output_dir = Path("rf_return_outputs")
...
for coin in coins:
    print(f"\n{'='*80}")
    print(f"RUNNING RANDOM FOREST RETURN PREDICTION FOR {coin}")
    print(f"{'='*80}")
    ...
    for lookback in lookbacks:
        ...
        results_df["actual_log_return"] = y_test.values
        results_df["predicted_log_return"] = y_pred
        results_df["error"] = results_df["actual_log_return"] - results_df["predicted_log_return"]

        pred_file = preds_dir / f"{coin.lower()}_rf_logreturn_lb{lookback}.csv"
        ...
        subplot_data.append({
            ...
            "y_true": results_df["actual_log_return"].values,
            "y_pred": results_df["predicted_log_return"].values,
            "rmse": rmse,
            "mae": mae,
            "mape": mape,
            "mda": mda
    ...
    fig.suptitle(
        f"Random Forest Forecast — {coin} Log Return",
        fontsize=14,
        y=0.98
    )
    ...
    combined_file = graphs_dir / f"{coin.lower()}_rf_logreturn_all_lookbacks.png"
...
print(f"\n{'='*80}")
print("FINAL RETURN SUMMARY")
print(f"{'='*80}")
print(summary_df.sort_values(["coin", "lookback"]))

summary_file = output_dir / "rf_logreturn_summary.csv"
