...
# ============================================================
# RANDOM FOREST — TRADING VOLUME PREDICTION
...
# Output folders
output_dir = Path("rf_volume_outputs")
...
for coin in coins:
    print(f"\n{'='*80}")
    print(f"RUNNING RANDOM FOREST VOLUME PREDICTION FOR {coin}")
    print(f"{'='*80}")
    ...
    for lookback in lookbacks:
        ...
        mda = volume_mda(y_test.values, y_pred)
        ...
        results_df["error"] = results_df["actual_volume"] - results_df["predicted_volume"]

        pred_file = preds_dir / f"{coin.lower()}_rf_volume_lb{lookback}.csv"
        ...
        subplot_data.append({
            ...
            "y_true": results_df["actual_volume"].values,
            "y_pred": results_df["predicted_volume"].values,
            "rmse": rmse,
            "mae": mae,
            "mape": mape,
            "mda": mda
    ...
    fig.suptitle(
        f"Random Forest Forecast — {coin} Trading Volume",
        fontsize=14,
        y=0.98
    )
    ...
    combined_file = graphs_dir / f"{coin.lower()}_rf_volume_all_lookbacks.png"
...
print(f"\n{'='*80}")
print("FINAL VOLUME SUMMARY")
print(f"{'='*80}")
print(summary_df.sort_values(["coin", "lookback"]))

summary_file = output_dir / "rf_volume_summary.csv"
