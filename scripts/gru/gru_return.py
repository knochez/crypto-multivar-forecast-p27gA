"""
gru_return.py — BTC Log Return forecasting using GRU
=====================================================
...
GRU settings for return:
  - Same aggressive settings as lstm_return.py (128 hidden, OneCycleLR)
  - Reduced max_lr=0.005 — return series is noisier than price
  - GRU fewer parameters than LSTM may reduce overfitting on noisy targets
"""
...
RESULTS_DIR  = "results_gru_return"
...
class GRUForecaster(nn.Module):
    ...
    Output is unbounded — no sigmoid/tanh — since returns can be positive or negative.
    """
...
# Trains GRU with OneCycleLR warmup and early stopping, returns best model
...
def plot_predictions(dates, yt, yp, lb, m):
    ...
    ax.plot(dates, yp, color="#D85A30", linewidth=1.1,
            ...
            label="Predicted btc log return — GRU model one-day-ahead forecast")
    ax.set_title(
...
def main():
    ...
    for i, lb in enumerate(LOOKBACKS, 1):
        ...
        torch.save(model.state_dict(), os.path.join(
            DIRS["model_weights"],
            f"Model_Weights_GRU_Return_lookback_{lb}d.pt"))
    ...
    csv = os.path.join(DIRS["summary"], "Results_GRU_Return.csv")
