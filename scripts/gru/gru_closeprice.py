"""
gru_price.py — BTC Close Price forecasting using GRU
=====================================================
Target   : close (next day BTC price in USD)
...
GRU vs LSTM for price:
  - GRU has fewer parameters (no separate memory cell) making it
    faster to train and less prone to overfitting on moderate datasets
  - Uses same aggressive settings as lstm_price.py (128 hidden units,
    OneCycleLR, BatchNorm) since price benefits from complex architecture
  - GRU update and reset gates replace the 3-gate LSTM mechanism
"""
...
TARGET       = "close"
TARGET_LABEL = "BTC Close Price (USD)"
TARGET_UNIT  = "Price (USD)"
RESULTS_DIR  = "results_gru_price"
...
# Plots actual vs predicted price with regime shading, error fill, metrics in title
def plot_predictions(dates, yt, yp, lb, m):
    fig, ax = plt.subplots(figsize=(13, 5))
    ax.axvspan(np.datetime64(BULL_START), np.datetime64(BULL_END),
               alpha=0.06, color="green", label="Bull regime — period of rising BTC price (Apr–Oct 2025)")
    ax.axvspan(np.datetime64(BEAR_START), np.datetime64(BEAR_END),
               alpha=0.06, color="red",   label="Bear regime — period of declining BTC price (Oct 2025–Jan 2026)")
    ...
    ax.plot(dates, yt, color="#185FA5", linewidth=1.4,
            label="Actual btc close price (usd) — true daily closing price from market data")
    ax.plot(dates, yp, color="#D85A30", linewidth=1.1,
            linestyle="--", alpha=0.85,
            label="Predicted btc close price (usd) — GRU model one-day-ahead forecast")
    ax.set_title(
    ...
    fig.savefig(os.path.join(DIRS["predictions"],
                f"Price_Forecast_lookback_{lb}d.png"), dpi=150)
    plt.close(fig)
    print(f"  Saved prediction plot -> {DIRS['predictions']}/Price_Forecast_lookback_{lb}d.png")
...
def plot_combined(all_preds):
    ...
    for ax, r in zip(axes.flatten(), all_preds):
        ...
        ax.set_title(
            ...
            fontsize=9)
        ax.set_xlabel("Date", fontsize=9); ax.set_ylabel("Price (USD)", fontsize=9)
...
def plot_summary(results_df):
    ...
    fig.suptitle(
        ...
        fontsize=10, y=1.02)
    fig.tight_layout()
    fig.savefig(os.path.join(DIRS["summary"], "Performance_Summary_Price.png"),
                dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"  Saved summary chart -> {DIRS['summary']}/Performance_Summary_Price.png")
...
def main():
    ...
    for i, lb in enumerate(LOOKBACKS, 1):
        ...
        torch.save(model.state_dict(), os.path.join(
            DIRS["model_weights"],
            f"Model_Weights_GRU_Price_lookback_{lb}d.pt"))
    ...
    csv = os.path.join(DIRS["summary"], "Results_GRU_Price.csv")
